...
# CNN scoring option for GNINA
cnn_scoring = "none"
if page_mode == "gnina":
    cnn_scoring = st.selectbox(
        "CNN Scoring Mode (GNINA only)",
        ["none", "rescore", "refinement", "all"],
        index=0,
        key=f"{state_prefix}_cnn_scoring",
        help="CNN scoring options: none (SMINA empirical scoring only), rescore (default CNN rescore - fast), refinement (CNN during refinement - slower), all (CNN for all steps - slowest but most accurate)"
    )
...
if run_btn:
    ...
    if receptor_path is None or not receptor_path.exists():
        ...
        st.stop()
    if not ligand_paths:
        ligand_error = "No ligand files found. Upload ligand files first." if page_mode == "gnina" else "No ligand files found. Upload ligand PDBQT files first."
        st.error(ligand_error)
        st.stop()

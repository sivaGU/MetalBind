...
def render_home_page():
    st.header("Welcome to ZincBind")
    st.write(
        "ZincBind streamlines AutoDock4 map-based docking for metalloprotein projects. "
    ...
    st.markdown(
        "1. Choose a working directory (ZincBind will create the required folders there).\n"
    ...
    st.write(
        "The CLI entry point (`python ZincBind.py --cli ...`) shares the same code paths as the GUI. "
        "Use it to script batch runs after maps are prepared."
    )

def render_documentation_page():
    st.header("ZincBind — Documentation")
    st.write(
        "Welcome to the documentation tab! Below is a detailed, step-by-step guide to the "
        "ZincBind workflow so you know what each section does and how to use it effectively."
    )

    st.subheader("① Prepare Your Workspace")
    st.markdown(
        "**1. Choose a working directory.**\\n"
        "ZincBind creates `prepared_ligands/`, `ad4_maps/`, and `outputs/` inside the folder you set. "
    ...
    st.markdown(
        ...
        "Use the uploader or a path. ZincBind normalizes oxygen labels (O → OA) and keeps coordinates intact."
    )
    st.markdown(
        "**2. Prepare ligands.**\\n"
    ...
    st.markdown(
        "ZincBind wraps AutoGrid4 to create or update map files. The workflow validates inputs before launching the executable:\\n"
    ...
    st.markdown(
        "**1. Click *Run Docking*.**\\n"
        "ZincBind queues each ligand, calls the appropriate executable (Vina or AD4), captures stdout/stderr, and displays live status." 
    )
    st.markdown(
...
def _run_cli():
    parser = argparse.ArgumentParser(description="ZincBind CLI (uses GUI code paths)")
...
if "current_page" not in st.session_state:
    st.session_state.current_page = "ZincBind Demo"
...
state_prefix = "demo" if page == "ZincBind Demo" else page_mode
...
with st.expander("Configuration", expanded=True):
    ...
    with c2:
        ...
        grid_disabled = page == "ZincBind Demo"
...
params_disabled = page == "ZincBind Demo"
with p1:
